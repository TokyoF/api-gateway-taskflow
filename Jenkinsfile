...
pipeline {
    agent any
    environment {
        DOCKER_IMAGE = 't0ky0le/api-gateway:1.0' // Nombre de la imagen en Docker Hub
    }
    stages {
        stage('Checkout') {
            steps {
                echo 'Clonando el repositorio...'
                git branch: 'main', url: 'https://github.com/TokyoF/api-gateway-taskflow.git' // Cambia con tu URL
            }
        }
        stage('Build') {
            steps {
                echo 'Construyendo el proyecto con Maven...'
                bat './mvnw.cmd clean package -DskipTests' // Compila usando bat para Windows
            }
        }
        stage('Docker Build') {
            steps {
        ...
        stage('Docker Compose Down') {
            steps {
                ...
                bat 'docker rm -f api-gateway || true' // Elimina el contenedor si existe
